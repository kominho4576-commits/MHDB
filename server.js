...
io.on('connection', (socket) => {
  ...
  socket.on('createPartyRoom', ({ code, playerName, avatarId, title, monsterId, monsterName, monsterHp, monsterTime, monsterTier }) => {
  ...
  socket.on('startPartyBattle', ({ roomCode, monsterId, monsterName, monsterHp, monsterTime, monsterTier }) => {
    const r = partyRooms[roomCode];
    if(!r) return;
    if(r.hostId !== socket.id) return;
