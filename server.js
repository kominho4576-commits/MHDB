...
function endPartyBattle(code, win, reason) {
  ...
  if (r.isRaid === true) emitRaidRoomList(r.monsterId || '');
  cleanupRoomLater(partyRooms, code, 1200);
  if (r.isRaid === true) {
    setTimeout(() => emitRaidRoomList(r.monsterId || ''), 1250);
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('createPartyRoom', (payload = {}) => {
    ...
    if (partyRooms[code] || raidRooms[code]) return socket.emit('roomError', 'Room code conflict, retry');
  ...
  socket.on('createRoom', () => {
    return socket.emit('roomError', 'Legacy raid path disabled. Use createRaidRoom.');
  });

  socket.on('joinRoom', () => {
    return socket.emit('roomError', 'Legacy raid path disabled. Use joinRaidRoom.');
  });

  socket.on('rejoinRoom', () => {
    return socket.emit('roomError', 'Rejoin disabled. Please create or join a new room.');
  });

  socket.on('startBattle', () => {
    return socket.emit('roomError', 'Legacy raid path disabled. Use startRaidBattle.');
  });

  socket.on('dealDamage', () => {
    return socket.emit('roomError', 'Legacy raid path disabled.');
  });

  socket.on('counterDmg', () => {
    return socket.emit('roomError', 'Legacy raid path disabled.');
  });
...
function refreshAllInventoryViews() {
  io.emit('inventorySyncHint', { at: Date.now() });
}

io.on('connection', (socket) => {
  socket.on('claimInventorySync', () => {
    socket.emit('inventorySyncHint', { at: Date.now() });
  });
});
