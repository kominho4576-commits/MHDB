...
function buildPartyRewards(room) {
  const members = Math.max(1, (room.members && room.members.length) || 1);
  const sizeMultiplier = 1 + ((members - 1) * 0.12);
  const zennyBase = Math.max(0, Number(room.monsterBaseZenny) || 0);
  const bonusZenny = Math.floor(Math.random() * 35);
  const scaledZenny = Math.floor((zennyBase + bonusZenny) * sizeMultiplier);
  const rewards = [];
  if (scaledZenny > 0) rewards.push({ type:'zenny', name:'Zenny', count: scaledZenny });
  if (room.monsterBone) {
    const matCount = members >= 4 ? 2 : 1;
    rewards.push({ type:'material', key: room.monsterBone, name: room.monsterBone, count: matCount });
  }
  return rewards;
}
function buildRank1Bonus(room) {
  const zennyBase = Math.max(0, Number(room.monsterBaseZenny) || 0);
  const rewards = [];
  const bonusZenny = Math.max(20, Math.floor(zennyBase * 0.25));
  if (bonusZenny > 0) rewards.push({ type:'zenny', name:'Zenny', count: bonusZenny });
  if (room.monsterBone) rewards.push({ type:'material', key: room.monsterBone, name: room.monsterBone, count: 1 });
  return rewards;
}
...
function endPartyBattle(code, win, reason) {
  ...
  const rank1Bonus = (win && members.length) ? { playerId: members[0].id, rewards: buildRank1Bonus(r) } : null;
...
function getRoomList() {
  return Object.values(partyRooms)
    ...
    .map(r => ({
      ...
      monsterId: r.monsterId||'',
      monsterName: r.monsterName||'',
      monsterHp: Number(r.monsterHp||0) || 0,
...
io.on('connection', (socket) => {
  ...
  socket.on('createPartyRoom', ({ code, playerName, avatarId, title, monsterId, monsterName, monsterHp, monsterTime, monsterTier, monsterBone, monsterBaseZenny }) => {
    ...
    socket.emit('partyRoomCreated', {
      ...
      monsterHp: Number(monsterHp||0) || 0,
  ...
  socket.on('startPartyBattle', ({ roomCode, monsterId, monsterName, monsterHp, monsterTime, monsterTier, monsterBone, monsterBaseZenny }) => {
    ...
    const baseHp = Number(monsterHp || r.monsterHp || 10000) || 10000;
    const extraMembers = Math.max(0, (r.members?.length || 1) - 1);
    // Party hunt HP rule: +50% base HP per additional hunter.
    const hp = Math.floor(baseHp * (1 + extraMembers * 0.5));
