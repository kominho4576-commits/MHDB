...
function getRoomList() {
  return Object.values(partyRooms)
    ...
    .map(r => ({
      ...
      monsterHp: computePartyMonsterHp(r.monsterBaseHp || r.monsterHp || 0, r.members.length),
    }));
}
...
io.on('connection', (socket) => {
  ...
  socket.on('createPartyRoom', ({ code, playerName, avatarId, title, monsterId, monsterName, monsterBaseHp, monsterHp, monsterTime, monsterTier }) => {
    if(partyRooms[code]) return socket.emit('roomError', 'Room code conflict, retry');
    partyRooms[code] = {
      code,
      monsterId, monsterName, monsterBaseHp: Number(monsterBaseHp||monsterHp||0), monsterHp: Number(monsterHp||0), monsterTime, monsterTier: monsterTier||1,
      hostId: socket.id,
  ...
  socket.on('startPartyBattle', ({ roomCode, monsterId, monsterName, monsterBaseHp, monsterHp, monsterTime, monsterTier }) => {
    const r = partyRooms[roomCode];
    if(!r) return;
    if(r.hostId !== socket.id) return;
