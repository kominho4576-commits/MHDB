...
const partyRooms = {};
const raidRooms = {};
const partyFirstClearHistory = new Set();

function computeSoloMonsterHp(baseHp) {
...
function computePartyMonsterHp(baseHp, memberCount) {
  ...
  return Math.floor(computeSoloMonsterHp(baseHp) * (1 + 0.5 * (count - 1)));
}
...
function getPartyAchievementKey(room, member) {
  const roomMonster = String(room.monsterId || room.monsterName || 'party').trim().toLowerCase();
  const playerName = String(member?.name || 'player').trim().toLowerCase();
  return `${playerName}::${roomMonster}`;
}
...
function buildRewards(room, winnerId) {
  ...
  const rankedMembers = room.members.map(serializeMember).sort((a, b) => (b.damage || 0) - (a.damage || 0));
  const placementById = {};
  rankedMembers.forEach((member, idx) => { placementById[member.id] = idx + 1; });
  const rewards = {};
  room.members.forEach((m) => {
    if (m.claimed === false || m.leftBattle === true) return;
    let zenny = Math.floor(baseZenny * bonusMultiplier);
    let mats = materialCount;
    const placement = placementById[m.id] || 0;
    ...
    if (room.roomType === 'party') {
      const key = getPartyAchievementKey(room, m);
      if (!partyFirstClearHistory.has(key)) {
        partyFirstClearHistory.add(key);
        achievements.push('Party Pioneer');
      }
    }
    rewards[m.id] = {
      zenny,
...
function getRoomList() {
  return Object.values(partyRooms)
    ...
    .map((r) => ({
      ...
      soloHp: computeSoloMonsterHp(r.monsterBaseHp || 1),
      partyHp: computePartyMonsterHp(r.monsterBaseHp || 1, r.members.length),
...
io.on('connection', (socket) => {
  ...
  socket.on('createPartyRoom', (payload = {}) => {
    ...
    socket.emit('partyRoomCreated', {
      ...
      monsterBaseHp: partyRooms[code].monsterBaseHp,
      soloHp: computeSoloMonsterHp(partyRooms[code].monsterBaseHp),
      partyHp: computePartyMonsterHp(partyRooms[code].monsterBaseHp, partyRooms[code].members.length),
      monsterTime: partyRooms[code].monsterTime,
  ...
  socket.on('joinPartyRoom', (payload = {}) => {
    ...
    socket.emit('partyRoomCreated', {
      ...
      monsterId: r.monsterId,
      monsterName: r.monsterName,
      monsterBaseHp: r.monsterBaseHp,
      soloHp: computeSoloMonsterHp(r.monsterBaseHp),
      partyHp: computePartyMonsterHp(r.monsterBaseHp, r.members.length),
      monsterTime: r.monsterTime,
      members: r.members.map(serializeMember),
    });
  ...
  socket.on('startPartyBattle', (payload = {}) => {
    ...
    io.to(roomCode).emit('partyBattleStarted', {
      ...
      soloHp: computeSoloMonsterHp(baseHp),
      partyHp: hp,
      monsterHp: hp,
      monsterTime: time,
  ...
  socket.on('startBattle', ({ roomCode, battle } = {}) => {
    ...
    r.rewardZenny = battle && battle.isDragon ? 350 : 180;
