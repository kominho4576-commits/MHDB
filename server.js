...
function emitPartyRoomState(code) {
  ...
  io.to(code).emit('partyBattleState', {
    roomCode: code,
    roomType: r.roomType || 'party',
    isDragon: r.isDragon === true,
    monsterId: r.monsterId || '',
    monsterName: r.monsterName || '',
    monsterBg: r.monsterBg || '',
...
function endPartyBattle(code, win, reason) {
  ...
  io.to(code).emit('partyBattleEnded', {
    ...
    monsterId: r.monsterId || '',
    monsterName: r.monsterName || '',
    rewardMaterialKey: r.rewardMaterialKey || '',
    battle: {
      currentHp: r.battle.currentHp,
      maxHp: r.battle.maxHp,
    },
...
function getRoomList(options = {}) {
  const type = options.roomType || '';
  const monsterId = options.monsterId || '';
  return Object.values(partyRooms)
    .filter((r) => (!r.battle || !r.battle.active))
    .filter((r) => (!type || (r.roomType || 'party') === type))
    .filter((r) => (!monsterId || r.monsterId === monsterId))
    .map((r) => ({
      code: r.code,
      roomType: r.roomType || 'party',
      isDragon: r.isDragon === true,
...
io.on('connection', (socket) => {
  socket.on('getRooms', () => {
    socket.emit('roomList', getRoomList({ roomType: 'party' }));
  });

  socket.on('getRaidRooms', (payload = {}) => {
    socket.emit('raidRoomList', getRoomList({ roomType: 'raid', monsterId: String(payload.monsterId || '').trim() || '' }));
  });
  ...
  socket.on('createPartyRoom', (payload = {}) => {
    ...
    partyRooms[code] = {
      code,
      hostId: socket.id,
      roomType: payload.roomType === 'raid' ? 'raid' : 'party',
      isDragon: payload.isDragon === true,
      monsterId: payload.monsterId || '',
      monsterName: payload.monsterName || '',
      monsterBg: payload.monsterBg || '',
      monsterBaseHp,
      ...
      rewardMaterialKey: payload.monsterBoneKey || '',
      members: [
        {
          id: socket.id,
    ...
    socket.emit('partyRoomCreated', {
      code,
      roomType: partyRooms[code].roomType,
      isDragon: partyRooms[code].isDragon,
      monsterId: partyRooms[code].monsterId,
      monsterName: partyRooms[code].monsterName,
      monsterBg: partyRooms[code].monsterBg,
  ...
  socket.on('kickMember', ({ roomCode, targetId } = {}) => {
    ...
    io.to(targetId).emit('kicked', { roomCode, playerName: target.name, roomType: r.roomType || 'party', refundType: (r.roomType === 'raid' ? 'ticket' : 'paintball') });
  ...
  socket.on('startPartyBattle', (payload = {}) => {
    ...
    r.roomType = payload.roomType || r.roomType || 'party';
    r.isDragon = payload.isDragon === true || r.isDragon === true;
    r.monsterId = payload.monsterId || r.monsterId;
    r.monsterName = payload.monsterName || r.monsterName;
    r.monsterBg = payload.monsterBg || r.monsterBg || '';
    ...
    r.rewardMaterialKey = payload.monsterBoneKey || r.rewardMaterialKey || '';
    r.members.forEach((m) => {
      m.damage = 0;
      m.active = true;
      m.connected = true;
